...
const NWGoalChart = props => {
  ...
  useEffect(() => {
    ...
    const data = [{ name: 'Current Net Worth', y: netWorthVal, color: 'rgb(26, 171, 152)' }];
    if (percReached < 100) {
      data.push({ name: 'Remaining', y: props.netWorthGoal - netWorthVal, color: 'rgb(15, 119, 147)' });
    }
    setPercent(percReached);
    setDataPoints(data);
  }, [props.netWorthData, props.netWorthGoal]);
  ...
  return (
    <div className={props.small ? classes.SmallChartContainer : classes.ChartContainer}>
      ...
      <h1>You've reached {percent}% of your goal</h1>
    </div>
  );
};
